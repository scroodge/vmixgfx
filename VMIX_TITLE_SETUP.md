...
**Важно:** JSON endpoint работает через **HTTP polling** (периодические HTTP запросы), а не через WebSocket. vMix автоматически запрашивает данные с заданным интервалом (обычно 1-2 секунды). WebSocket используется только для overlay в браузере и не требуется для vMix Title.
...
4. **Важно:** Настройте интервал обновления (Refresh Interval):
   - Рекомендуется: **1-2 секунды** для реального времени
   - Минимум: **1 секунда** (для быстрого обновления)
   - Максимум: **5 секунд** (если не критично)

5. Нажмите **"OK"** для сохранения источника данных

**Примечание:** JSON endpoint работает независимо от WebSocket. vMix будет автоматически запрашивать обновленные данные с заданным интервалом. WebSocket используется только для overlay в браузере, но не требуется для vMix Title.
...
- JSON endpoint всегда возвращает актуальные данные из сервера (не требует WebSocket)
- Данные обновляются при каждом запросе к endpoint, независимо от WebSocket подключения
...
1. **Проверьте интервал обновления в vMix Data Source:**
   - Откройте Data Sources Manager
   - Выберите ваш JSON data source
   - Убедитесь, что установлен интервал обновления (Refresh Interval)
   - Рекомендуется: 1-2 секунды для реального времени
   - Если интервал не установлен или слишком большой, данные будут обновляться медленно

2. **Проверьте, что сервер запущен и доступен:**
   - Откройте URL в браузере: `http://localhost:8000/api/match/1/data.json`
   - Должен вернуться валидный JSON массив

3. **Проверьте URL в Data Source:**
   - URL должен быть точно таким же, как в браузере
   - Убедитесь, что используется правильный match_id

4. **Проверьте формат JSON:**
   - JSON должен быть валидным массивом объектов
   - Формат: `[{...}]` (не просто `{...}`)

5. **Важно:** JSON endpoint работает через **HTTP polling** (периодические запросы), а не через WebSocket
   - WebSocket используется только для overlay в браузере
   - vMix Title использует HTTP запросы с заданным интервалом
   - Данные обновляются при каждом запросе к endpoint
