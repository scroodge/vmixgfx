...
| **Шары 1.Text** | `{home_score}` | Счет первого игрока |
| **Фамилия 2.Text** | `{away_name}` | Имя второго игрока (гостевая команда) |
| **Шары 2.Text** | `{away_score}` | Счет второго игрока |
| - | `{home_match_score}` | Общий счет матча (игры выиграны) для первого игрока |
| - | `{away_match_score}` | Общий счет матча (игры выиграны) для второго игрока |
| - | `{period}` | Номер периода/игры |
